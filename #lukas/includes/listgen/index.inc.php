...
function fetchRecent($conn, $user_id) {
    ...
    if(isset($following[0])) {
        ...
        if($num > 0) {
            ...
            $date_lim = date('Y-m-d H:i:s', strtotime('-1 week'));
            $sql = "SELECT `logs`.`user_id` AS `user_id`, `users`.`uid` AS `user_uid`, `users`.`name` AS `username`, `logs`.`date` AS `date`, `logs`.`like` AS `like`, `logs`.`rating` AS `rating`, `logs`.`rewatch` AS `rewatch`, '' AS `review_text`, 0 AS `spoilers`, `logs`.`item_id` AS `item_id`, `items`.`type` AS `item_type`, `items`.`uid` AS `item_uid`, `items`.`name` AS `item_name`, `items`.`year` AS `item_year` 
            FROM `logs` 
            ...
            UNION 
            SELECT `reviews`.`user_id` AS `user_id`, `users`.`uid` AS `user_uid`, `users`.`name` AS `username`, `reviews`.`date` AS `date`, `reviews`.`like` AS `like`, `reviews`.`rating` AS `rating`, 0 AS `rewatch`, `reviews`.`text` AS `review_text`, `reviews`.`spoilers` AS `spoilers`, `reviews`.`item_id` AS `item_id`, `items`.`type` AS `item_type`, `items`.`uid` AS `item_uid`, `items`.`name` AS `item_name`, `items`.`year` AS `item_year` 
            FROM `reviews` 
